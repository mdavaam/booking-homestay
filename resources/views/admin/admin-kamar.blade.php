...
<body>
                    ...
                    <div class="card-body">
                        ...
                        <div class="mb-3">
                            <input type="text" id="roomType" class="form-control"
                                placeholder="cari berdasarkan tipe kamar atau nomor kamar...">
                        </div>
                                ...
                                <tbody class="text-center align-middle">
                                    @foreach ($kamars as $kamar)
                                        <tr>
                                            ...
                                            <td>
                                                <span
                                                    class="badge {{ $kamar->status == 'tersedia' ? 'bg-success' : 'bg-danger' }}">
                                                    {{ $kamar->status }}
                                                </span>
                                            </td>
                                            ...
                                            <td>
                                                ...
                                                <form action="{{ route('admin.kamardalam.destroy', $kamar->id) }}"
                                                    method="POST" style="display:inline-block;">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit" class="btn btn-danger btn-sm"
                                                        onclick="return confirm('Yakin ingin menghapus kamar ini?')">Delete</button>
                                                </form>
                                            </td>
                                        </tr>
                                    @endforeach
                ...
                <form action="{{ route('admin.kamardalamStore') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="file" class="form-control" id="photo_utama" name="photo_utama"
    ...
    @foreach ($kamars as $kamar)
        <div class="modal fade" id="addPhoto{{ $kamar->id }}" tabindex="-1"
            aria-labelledby="addPhotoLabel{{ $kamar->id }}" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    <form action="{{ route('admin.addPhoto', ['nama_kamar' => $kamar->nama_kamar]) }}" method="POST"
                        enctype="multipart/form-data">
                        @csrf
                        <label for="photo_kamar{{ $kamar->id }}" class="form-label">Pilih Foto Kamar</label>
                        <input type="file" id="photo_kamar{{ $kamar->id }}" name="photo_kamar[]"
    ...
    <!-- Modal Edit -->
    @foreach ($kamars as $kamar)
        <div class="modal fade" id="editModal{{ $kamar->id }}" tabindex="-1"
            aria-labelledby="editModalLabel{{ $kamar->id }}" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        @isset($kamar)
                            <form action="{{ route('admin.kamar.update', $kamar->id) }}" method="POST"
                                ...
                                {{-- Tipe Kamar --}}
                                <div class="mb-3">
                                    <label for="jenis_kamar{{ $kamar->id }}" class="form-label">Tipe Kamar</label>
                                    <input type="text" name="jenis_kamar" id="jenis_kamar{{ $kamar->id }}"
                                        class="form-control" value="{{ old('jenis_kamar', $kamar->jenis_kamar) }}"
                                        required>
                                </div>
                                ...
                                <div class="mb-3">
                                    ...
                                    <input type="text" name="nama_kamar" id="nama_kamar{{ $kamar->id }}"
                                        class="form-control" value="{{ old('nama_kamar', $kamar->nama_kamar) }}"
                                        required>
                                </div>

                                {{-- Harga Per Malam --}}
                                <div class="mb-3">
                                    <label for="harga_permalam{{ $kamar->id }}" class="form-label">Harga Per
                                        Malam</label>
                                    <input type="number" name="harga_permalam" id="harga_permalam{{ $kamar->id }}"
                                        class="form-control" value="{{ old('harga_permalam', $kamar->harga_permalam) }}"
                                        required min="0">
                                </div>
                                ...
                                {{-- Upload Foto Kamar --}}
                                <div class="mb-3">
                                    <label for="photo_path{{ $kamar->id }}" class="form-label">Foto Kamar (Maksimal 4
                                        Gambar)</label>
                                    <input type="file" name="photo_path[]" id="photo_path{{ $kamar->id }}"
                                        class="form-control" multiple accept="image/*" onchange="restrictFiles(this, 4)">
                                ...
                                <div class="mb-3">
                                    ...
                                    <select name="status" id="status{{ $kamar->id }}" class="form-control" required>
                                        <option value="tersedia"
                                            {{ old('status', $kamar->status) == 'tersedia' ? 'selected' : '' }}>Tersedia
                                        </option>
                                        <option value="tidak Tersedia"
                                            {{ old('status', $kamar->status) == 'tidak Tersedia' ? 'selected' : '' }}>Tidak
                                            Tersedia</option>
                                    </select>
                                </div>
