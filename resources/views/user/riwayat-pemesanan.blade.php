                    ...
                    <div class="sidebar">
                        <div class="card-body p-0">
                            ...
                            <a href="{{ route('myBookings') }}"
                                class="sidebar-item {{ request()->routeIs('myBookings') ? 'active' : '' }}"
                                    ...
                                    @foreach ($pesananku as $pesanan)
                                        <div class="booking-item">
                                            ...
                                            @if ($pesanan->kamar && $pesanan->kamar->photo_utama)
                                                <img src="{{ asset('storage/' . $pesanan->kamar->photo_utama) }}"
                                                    alt="Kamar {{ $pesanan->kamar->nama_kamar }}" class="booking-img">
                                            @else
                                                <img src="{{ asset('images/default-room.jpg') }}" alt="Default Kamar"
                                                    class="booking-img">
                                            @endif

                                            <div class="booking-details">
                                                <strong>Nama Kamar:</strong> {{ $pesanan->kamar->nama_kamar ?? '-' }}
                                                <br>
                                                <strong>Kode Transaksi:</strong> {{ $pesanan->kode_transaksi }} <br>
                                                <strong>Total Pembayaran:</strong>
                                                Rp{{ number_format($pesanan->total_harga, 0, ',', '.') }} <br>
                                                <strong>Status:</strong>
                                                <span
                                                    class="{{ $pesanan->status === 'pending' ? 'status-pending' : '' }}">
                                                    {{ $pesanan->status }}
                                                </span>
                                            </div>
                                        </div>
                                        @if ($pesanan->status === 'pending')
                                            <div class="booking-actions">
                                                <form action="{{ route('batalkan.pesanann', $pesanan->id) }}"
                                                ...
                                                <a href="{{ route('pembayaran.lanjut', $kodeUrl) }}"
